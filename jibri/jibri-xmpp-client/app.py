...
default_archive_host="sugar-alf-demo1.hosts.webrecs.com"   #default host for archiving recordings
...
def finalize_recording(referrer, cleanup):
    global pjsua_flag
    global recording_directory
    cleanup_flag= '-c' if (cleanup ) else "" 
    #run any final processing tasks before finishing up, if not in pjsua mode
    if not pjsua_flag:
        call([finalize_recording_script,recording_directory, referrer, cleanup_flag ])
...
def kill_ffmpeg_process():
    try:
        ...
        os.kill(ffmpeg_pid, 15 ) #SIGTERM
    except Exception as e:
        logging.info ("Exception killing ffmpeg %s " %e)  
        return False
    return True
...
def jibri_start_callback(client, url, recording_mode='file', stream_id='', sipaddress=None, displayname=None, room=None, token='token', backup='', recording_name='', now=True):
    global js
    global loop
    global opts
    global client_opts
    ...
    if now: # Record now
        if not recording_name:
            ...
            recording_name = url[url.find('://')+3:] #webrecs change since not always https
    ...
    active_call['skip_check_audio_stream']=True #webrecs 
...
def launch_ffmpeg(url,recording_path='',token='',stream_id='', backup='', prefix='' ):
    if prefix: 
        #recording_file = recording_path +  prefix + datetime.now().strftime('-%Y%m%d%H%M%S.mp4')
        recording_file = recording_path +  prefix + '.mp4'
    else:
...
def jibri_stop_callback(status=None):
    ...
    active_client = None
    reset_recording()
    finalize_recording(default_archive_host, True)
    release_recording()
    ...
    while True:
        ...
        while (result and selenium_result):
            ...
            if pjsua_flag:
                ...
                result = True if (retry_value == "norecord") else check_ffmpeg_running(False) #if the message is "norecord" skip the ffmpeg check
...
def start_sleekxmpp(hostname, loop, selenium_lock, signal_queue,port=5222):
    global clients
    global client_opts
...
def parse_args (request, name, default):
...
def url_start_recording():
    global rest_token
    url = parse_args (request, 'url', '');
    stream = parse_args(request, 'stream', '')
    token = parse_args(request, 'token', '')
    now = True if parse_args (request, 'now', '').upper()=="TRUE" else False
#    if request.method == 'POST':
#        url = request.json['url']
#    else:
#        url = request.args['url']
    if not url :
        result = {'success': False, 'error':'Bad Parameters - url not defined', 'request':request.json }
        return jsonify(result)
    else:
        if rest_token == token:
            global selenium_lock
            recording_mode='file' if (stream=='') else 'stream'
            room=url[url.rindex("/"):]
            if selenium_lock.acquire(False):
                logging.info ("REST API Start, calling jibri_start_callback(None, %s, recording_mode=%s, stream_id=%s, room=%s, sipaddress='' , now=%s)"%(url, recording_mode, stream, room, now))
                retcode=jibri_start_callback(None, url, recording_mode=recording_mode, stream_id=stream, room=room, sipaddress='' , now=now)
                if retcode == 0:
...
def url_stop_recording():
    global default_archive_host
    referrer = request.referrer.split('/')[2] if request.referrer != None else  default_archive_host
    global selenium_lock
    reset_recording()
    finalize_recording(referrer, True)
    release_recording()
...
@app.route('/jibri/api/v1.0/pause', methods=['POST','GET'])
def url_pause_recording():
    global selenium_lock
    global default_archive_host
    referrer = request.referrer.split('/')[2] if request.referrer != None else  default_archive_host
    if selenium_lock.locked():
        kill_ffmpeg_process()
        logging.info ("Finalising recording for %s"%referrer)
        finalize_recording(referrer, False)
        success = True
        result = {'success': success}
        return jsonify(result)
    result = {'success': False, 'error':'Not locked'}
    return jsonify(result)

@app.route('/jibri/api/v1.0/record', methods=['POST','GET'])
def url_record():
    global recording_directory
    global selenium_lock
    global rest_token
    ...
    url = parse_args (request, 'url', '');
    token = parse_args(request, 'token', '')
    prefix = parse_args(request, 'prefix', 'webrecs' )
    if not url:
        result = {'success': False, 'error':'Bad Parameters url not defined', 'request':request.json}
        return jsonify(result)
    if not selenium_lock.locked():
        result = {'success': False, 'error':'Selenium is not running'}
        return jsonify(result)
    else:
        recording_name = url[url.find('://')+3:] 
        recording_path = recording_directory + '/'+recording_name + '/'
        try:
            os.makedirs(recording_path)
        except FileExistsError as exc:
            if os.path.isdir(recording_path):
                ...
                raise
        logging.info ("REST API record = launch_ffmpeg('' ,recording_path=%s, prefix=%s )"%(recording_path, prefix)) 
        launch_ffmpeg('' ,recording_path=recording_path, prefix=prefix )
        success = True
        result = {'success': success}
        return jsonify(result)
...
def url_health_check():
    ...
    result={'recording':selenium_lock.locked(), 'health':False, 'XMPPConnected':False, 'environment':current_environment}
    ...
    for hostname in candidate_hosts:
        ...
        if not 'url' in client_opts[hostname]:
            if 'xmpp_domain' in client_opts[hostname]:
                ...
                client_opts[hostname]['url'] = 'http://%s/%%SUBDOMAIN%%%%ROOM%%'%client_opts[hostname]['xmpp_domain']
            else:
...
#    loop.set_debug(True)
    ...
    for hostname in client_opts:
        ...
        loop.run_in_executor(None, start_sleekxmpp, hostname, loop, selenium_lock, queues[hostname])
