...
class ActionUserBot(sleekxmpp.ClientXMPP):
    ...
    def __init__(self, jid, password, other, room, nick, roompass, url, streamid):
    ...
    def send_custom_iq(self):
        ...
        iq['jibri']._setAttr('streamid', self.streamid )
...
if __name__ == '__main__':
    ...
    optp.add_option("-R", "--roompass", dest="roompass",
                    help="MUC password")
    optp.add_option("-s", "--streamid", dest="streamid",
                    help="Stream ID")
    ...
    if opts.streamid is None:
        opts.streamid = YSI

    # Setup the CommandBot and register plugins. Note that while plugins may
    # have interdependencies, the order in which you register them does
    # not matter.
    xmpp = ActionUserBot(opts.jid, opts.password, opts.other, opts.room, opts.nick, opts.roompass, opts.url, opts.streamid)
    ...
    if xmpp.connect(('jitsi.hosts.webrecs.com', 5222)):
